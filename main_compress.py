...
import kornia
from data.syncrgbd_dataloader_compress import SyncRGBD_Dataset
...
def main_worker(args):
    ...
    train_dataset = SyncRGBD_Dataset(root_dir="./dataset/", txt_file='./data/SyncRGBD_Train.txt', scale=scale, train=True)#SyncRGBD_Train
    val_SyncRGBD = SyncRGBD_Dataset(root_dir="./dataset/", txt_file='./data/SyncRGBD_Test.txt', scale=scale, train=False)

    val_Lu = Middlebury_dataset(root_dir='./Lu/', scale=scale)
    val_Midd = Middlebury_dataset(root_dir='./Middlebury/', scale=scale)
    val_NYU = CompreNYU_v2_datset(root_dir="./nyu_data/", scale=scale, train=False)
    test_minmax = np.load('%s/test_minmax.npy' % "./nyu_data/")
