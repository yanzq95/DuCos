...
import kornia
from data.syncrgbd_dataloader import SyncRGBD_Dataset#------------------------
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, sample in enumerate(train_loader):
        ...
        # ----- update ω -----
        # loss = L_rec + mu_gr * L_grad + lambda_cf * L_align
    ...
    ##################################################################
    # # ----- update λ, μ -----
    # eta_lambda_t = eta_lambda * (1 - epoch / args.epochs)
    # eta_mu_t = eta_mu * (1 - epoch / args.epochs)
    #
    # lambda_cf = lambda_cf + eta_lambda_t * L_align.item()
    # mu_gr = mu_gr + eta_mu_t * L_grad.item()
    #
    # lambda_cf = torch.clamp(lambda_cf, min=0.0)
    # mu_gr = torch.clamp(mu_gr, 0.0)
    ##################################################################
