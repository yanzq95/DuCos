...
def main_worker(args):
    ...
    if args.data_name == 'TOFDSR':
        if args.isNosiy == 0:
            from data.tofdsr_dataloader import TOFDSR_Dataset
            args.max_depth = 10.0
            train_dataset = TOFDSR_Dataset(root_dir="/opt/data/private/DSR/datasets/", scale=4, downsample='real',
                                          train=True, txt_file="./data/TOFDC_Filled_Train.txt")
            val_datasets = TOFDSR_Dataset(root_dir="/opt/data/private/DSR/datasets/", scale=4, downsample='real',
                                         train=False,  txt_file="./data/TOFDC_Filled_Test.txt")
        else:
            from data.tofdsr_dataloader_Noisy import TOFDSR_Dataset
            args.max_depth = 10.0
            train_dataset = TOFDSR_Dataset(root_dir="/opt/data/private/DSR/datasets/", scale=4, downsample='real',
                                          train=True, txt_file="./data/TOFDC_Filled_Train.txt", isBlur=True, isNoisy=True)
            val_datasets = TOFDSR_Dataset(root_dir="/opt/data/private/DSR/datasets/", scale=4, downsample='real',
                                         train=False,  txt_file="./data/TOFDC_Filled_Test.txt", isBlur=True, isNoisy=True)
        print('Dataset is TOFDSR')
    else:
    ...
    scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=args.decay, gamma=0.5)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        train_rmse, train_mae = train(train_loader, model, criterion, optimizer, epoch, args)
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, sample in enumerate(train_loader):
        ...
        error_str = '{:<5s} {} | {} | Loss = {:.4f}'.format('Train', epoch, current_time, loss.item())
